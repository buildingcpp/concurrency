...
[`work_contract_group.h`](src/library/work_contract/work_contract_group.h):
...
fixed array of contract *slots* and two `bcpp::signal_set` structures (from
`signal_tree`):
...
|---|---:|---:|---|
| Create | Free slot | `000` | `available_.select()` owns the slot; install `work_`; optionally apply Schedule. |
...
| Erase | `ER` or terminal derivative | next generation, `000`, free | Clear `work_`, store the incremented generation with all flags clear, then return the slot to `available_`. |
...
- If `work_` throws, `process_exception()` runs while the slot remains in its
...
- If the release handler throws, `process_exception()` runs while finalization
...
Validated by [`generation_test.cpp`](src/test/generation_test.cpp): a deterministic
