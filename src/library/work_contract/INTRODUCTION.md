...
bcpp::concurrency::work_contract_group group({.capacity_ = 512});
inbox pending;
...
```cpp
bcpp::concurrency::work_contract_group group({.capacity_ = 4096});
...
bcpp::concurrency::blocking_work_contract_group<> group({.capacity_ = 4096});
std::atomic<bool> running{true};
...
- The group owns the work, release, and exception callables in a fixed slot.
- Destroying a live handle requests release.
- Requesting release does not immediately destroy the callable.
- An executor processes release, calls that contract's release handler, destroys
  all three callables and their captured resources, bumps the slot generation,
...
Handlers are configured for each contract:
...
- that contract's exception handler receives the `exception_ptr`;
...
Handlers run on executor threads. With multiple workers, handlers for different
contracts can run concurrently, so shared state they touch must use the
application's normal synchronization.
...
```cpp
bcpp::concurrency::work_contract_group<64> group({.capacity_ = 100});
