...
namespace
{
    ...
    {
        constexpr auto capacity = bcpp::signal_tree<tree_depth>::capacity;
        constexpr auto leafCapacity = bcpp::signal_tree<0>::capacity;
        std::vector<std::uint64_t> probes;
    ...
    (
        std::vector<std::uint64_t> const & probes,
        std::uint64_t capacity
    ) -> std::vector<bcpp::signal_id>
    {
        std::vector<bcpp::signal_id> hints;
        hints.reserve(probes.size() + 3);
        hints.push_back(bcpp::signal_id::invalid());

        for (auto probe : probes)
            hints.push_back(bcpp::signal_id{probe});

        hints.push_back(bcpp::signal_id{capacity});
        hints.push_back(bcpp::signal_id{capacity + 17});
        return hints;
    }
    ...
    void set_selection_contract()
    {
        ...
        for (auto shardCount : std::array<std::size_t, 3>{1, 3, 9})
        {
            ...
            for (auto const & ready : sets)
                for (auto initialHint : hints)
                {
                    bcpp::signal_set<tree_depth> signals{capacity};

                    for (auto signal : ready)
                        test_support::require(signals.set(bcpp::signal_id{signal}),
