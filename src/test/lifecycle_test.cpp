...
{
    std::vector<bcpp::work_contract_id> released;
    bcpp::work_contract_group<64> group(
            {.capacity_ = 64},
            {.contractReleased_ = [&](auto id) { released.push_back(id); }});
    ...
    suite.check(released.size() == 1 && released.front().valid(), "release handler runs exactly once with a valid internal id");
...
void test_destructor_release(wc_test::suite & suite)
{
    ...
    bcpp::work_contract_group<64> group(
            {.capacity_ = 64},
            {.contractReleased_ = [&](auto id)
                {
                    ++released;
                    suite.check(id.valid(), "destructor-triggered release reports a valid internal id");
                }});
...
void test_move_semantics(wc_test::suite & suite)
{
    ...
    std::uint64_t runs{};
    auto source = group.create_contract([&] { ++runs; });
    bcpp::work_contract destination{std::move(source)};
    ...
    auto overwritten = group.create_contract([] {});
    ...
    suite.check(released.size() == 1 && released.front().valid(), "only the overwritten destination was released");
...
void test_self_control_and_context(wc_test::suite & suite)
{
    bcpp::work_contract_group<64> group({.capacity_ = 64});
    std::uint64_t runs{};
    bcpp::work_contract_id observed;

    suite.check(not bcpp::this_contract::is_executing(), "this_contract is inactive outside work");

    auto contract = group.create_contract(
            [&]
            {
                ++runs;
                suite.check(bcpp::this_contract::is_executing(), "this_contract is active inside work");
                observed = bcpp::this_contract::get_id();
                if (runs < 3)
    ...
    suite.check(not bcpp::this_contract::is_executing(), "this_contract context is cleared after work");
}
...
{
    bcpp::work_contract_group<64> group({.capacity_ = 64});
    bcpp::work_contract inner;
    bcpp::work_contract_id outerId;
    bcpp::work_contract_id innerId;
    std::uint64_t innerRuns{};

    auto outer = group.create_contract(
            [&]
            {
                outerId = bcpp::this_contract::get_id();
                inner.schedule();
                suite.check(group.execute_next_contract(), "work can synchronously execute another pending contract");
                suite.check(bcpp::this_contract::get_id() == outerId, "outer context is restored after nested execution");
            });
    ...
    inner = group.create_contract(
            ...
            {
                ++innerRuns;
                innerId = bcpp::this_contract::get_id();
            });
    ...
    suite.check(not bcpp::this_contract::is_executing(), "nested execution leaves no stale TLS context");
}
...
void test_work_exception_paths(wc_test::suite & suite)
{
    ...
    {
        std::uint64_t caught{};
        bcpp::work_contract_id caughtId;
        bcpp::work_contract_id executingId;
        std::string message;
        bcpp::work_contract_group<64> group(
                {.capacity_ = 64},
                {.contractException_ = [&](auto id, std::exception_ptr exception)
                    {
                        ++caught;
                        caughtId = id;
                        try
        ...
        auto contract = group.create_contract(
                [&]
                {
                    executingId = bcpp::this_contract::get_id();
        ...
        suite.check(caughtId == executingId, "the exception handler receives the executing contract's internal id");
...
{
    {
        bcpp::work_contract_group<64> group(
                {.capacity_ = 64},
                {.contractReleased_ = [](auto) { throw release_error{"release failed"}; }});
        auto contract = group.create_contract([] {});
        contract.release();
...

    {
        ...
        bcpp::work_contract_group<64> group(
                {.capacity_ = 64},
                {
                    .contractReleased_ = [](auto) { throw release_error{"handled release error"}; },
                    .contractException_ = [&](auto, std::exception_ptr exception)
                    {
                        ++caught;
                        try
                        {
        ...
        suite.check(caught == 1 && message == "handled release error", "release errors use the group exception handler");
