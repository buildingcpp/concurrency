...
namespace
{
    ...
    static_assert(bcpp::concurrency::signal_set<0>::required_tree_count(0) == 1);
    static_assert(bcpp::concurrency::signal_set<0>::required_tree_count(1) == 1);
    static_assert(bcpp::concurrency::signal_set<0>::required_tree_count(
            test_support::signal_tree<0>::capacity + 1) == 2);

    using external_signal_set = bcpp::concurrency::signal_set<0>;
    using external_tree_span = std::span<typename external_signal_set::tree_type>;
    using blocking_signal_set = bcpp::concurrency::signal_set<0, bcpp::synchronization_mode::blocking>;
    using shared_nonblocking_signal_set = bcpp::concurrency::shared_signal_set<0>;
    using shared_blocking_signal_set = bcpp::concurrency::shared_signal_set<
    ...
    static_assert(std::constructible_from<shared_blocking_signal_set, external_tree_span,
            typename shared_blocking_signal_set::shared_state_type &>);
    static_assert(std::same_as<shared_nonblocking_signal_set,
            bcpp::concurrency::signal_set<0, bcpp::synchronization_mode::non_blocking,
                    bcpp::concurrency::process_sharing::process_shared>>);
    ...
    {
        using signal_set_type = bcpp::concurrency::signal_set<N>;
        ...
        auto const firstSignal = bcpp::concurrency::signal_id{3};
        auto const secondSignal = bcpp::concurrency::signal_id{treeCapacity + 5};
        auto hint = bcpp::concurrency::signal_id{0};

        test_support::require(first.max() == (treeCapacity * trees.size()) - 1,
                "external storage must determine the complete physical capacity");
        test_support::require(first.set(secondSignal),
                "one view must publish into externally supplied trees");
        test_support::require(second.select(hint) == secondSignal,
                "another view must select from the same externally supplied trees");
        test_support::require(second.set(firstSignal),
                "the second view must publish into externally supplied trees");
        hint = bcpp::concurrency::signal_id{0};
        test_support::require(first.select(hint) == firstSignal,
                "the first view must observe publication through the second view");
    }
