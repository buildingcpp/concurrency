...
namespace
{
    ...
    using default_tree = bcpp::concurrency::signal_tree<bcpp::concurrency::default_signal_tree_traits>;
    ...
    static_assert(bcpp::concurrency::signal_tree_traits_concept<bcpp::concurrency::default_signal_tree_traits>);
    static_assert(not bcpp::concurrency::signal_tree_traits_concept<int>);
    static_assert(std::same_as<decltype(bcpp::concurrency::signal_tree{}), default_tree>);
    ...
    void basic_transitions()
    {
        ...
        auto const signal = bcpp::concurrency::signal_id{test_support::signal_tree<N>::capacity / 2};
    ...
    void occupancy_transitions()
    {
        ...
        tree_type tree;
        auto const firstSignal = bcpp::concurrency::signal_id{1};
        auto const secondSignal = bcpp::concurrency::signal_id{2};
    ...
    void fill_and_drain()
    {
        ...
        for (std::uint64_t index = 0; index < capacity; ++index)
            test_support::require(test_support::set_signal(tree, bcpp::concurrency::signal_id{index}),
                    "every clear signal must be publishable");

        for (std::uint64_t index = 0; index < capacity; ++index)
            test_support::require(not test_support::set_signal(tree, bcpp::concurrency::signal_id{index}),
    ...
    )
    {
        ...
        for (auto signal : ready)
            test_support::require(test_support::set_signal(tree, bcpp::concurrency::signal_id{signal}), caseName);

        auto hint = bcpp::concurrency::signal_id{initialHint};

        for (auto selected = expected.begin(); selected != expected.end(); ++selected)
        {
            test_support::require(test_support::select_signal(tree, hint) == bcpp::concurrency::signal_id{*selected}, caseName);
    ...
    void densest_selector_prefers_the_densest_root_child()
    {
        ...
        test_support::require(test_support::set_signal(tree, bcpp::concurrency::signal_id{0}),
                "sparse child signal must be publishable");
        test_support::require(test_support::set_signal(tree, bcpp::concurrency::signal_id{denseChildBase}),
                "dense child signal must be publishable");
        test_support::require(test_support::set_signal(tree, bcpp::concurrency::signal_id{denseChildBase + 1}),
                "dense child signal must be publishable");
        test_support::require(test_support::set_signal(tree, bcpp::concurrency::signal_id{denseChildBase + 2}),
                "dense child signal must be publishable");

        auto hint = bcpp::concurrency::signal_id{0};
        auto const [selected, resultingOccupancy] =
                tree.template select<bcpp::concurrency::densest_child_selector>(hint);
