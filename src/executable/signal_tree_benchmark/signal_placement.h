...
namespace adversarial_throughput_benchmark
{
    ...
    struct signal_tree_level_for_impl<RequiredCapacity, N, false>
    {
        ...
        <
            RequiredCapacity,
            N + 1,
            (bcpp::signal_tree<N + 1>::capacity >= RequiredCapacity)
        >::value;
    };
    ...
    <
        RequiredCapacity,
        0,
        (bcpp::signal_tree<0>::capacity >= RequiredCapacity)
    >::value;
    ...
    public:
        ...
        static constexpr auto physical_capacity   = bcpp::signal_tree<placement_tree_size>::capacity;
        ...
        {
            bcpp::signal_tree<placement_tree_size> placementTree;

            for (auto physical = 0ull; physical < physical_capacity; ++physical)
                placementTree.set(bcpp::signal_id{physical});
            for (auto logical = 0ull; logical < ScheduledSignalCount; ++logical)
            {
                auto hint = bcpp::signal_id{0};
                auto const physical = placementTree.template select<bcpp::densest_child_selector>(hint);
