...
namespace
{
    ...
    using blocking_signal_set = bcpp::signal_set<tree_depth, bcpp::synchronization_mode::blocking>;

    template <std::size_t tree_depth>
    using blocking_signal_tree = bcpp::signal_tree<tree_depth, bcpp::synchronization_mode::blocking>;


    template <typename T>
    concept has_timed_select = requires (T & signals, bcpp::signal_id & hint)
    {
        signals.select(hint, std::chrono::milliseconds{1});
    };

    template <typename T>
    concept has_try_select = requires (T & signals, bcpp::signal_id & hint)
    {
        signals.try_select(hint);
    };

    template <typename T>
    concept has_simple_signal_api = requires (T & signals, bcpp::signal_id signal, bcpp::signal_id & hint)
    {
        { signals.set(signal) } -> std::same_as<bool>;
        { signals.select(hint) } -> std::same_as<bcpp::signal_id>;
    };
    ...
    static_assert(not has_timed_select<bcpp::signal_set<0>>);
    static_assert(not has_timed_select<bcpp::signal_tree<0>>);
    static_assert(not has_try_select<bcpp::signal_tree<0>>);
    static_assert(has_simple_signal_api<bcpp::signal_set<0>>);
    static_assert(has_simple_signal_api<blocking_signal_set<0>>);
    static_assert(has_simple_signal_api<bcpp::signal_tree<0>>);
    ...
    void timed_select_returns_ready_signal()
    {
        blocking_signal_set<0> signals{64};
        auto hint = bcpp::signal_id{0};
        auto const ready = bcpp::signal_id{7};
    ...
    void timed_select_wakes_for_set()
    {
        blocking_signal_set<0> signals{64};
        std::latch entered{1};
        std::atomic<bcpp::signal_id::value_type> selected
        {
            static_cast<bcpp::signal_id::value_type>(bcpp::signal_id::invalid())
        };
        ...
        std::jthread waiter
        {
            [&]
            {
                auto hint = bcpp::signal_id{0};
                entered.count_down();
                selected.store(static_cast<bcpp::signal_id::value_type>(signals.select(hint, 5s)));
            }
        };

        entered.wait();
        test_support::require(signals.set(bcpp::signal_id{9}), "cross-thread signal must be publishable");
        waiter.join();
    ...
    void zero_timeout_is_one_immediate_attempt()
    {
        ...
        blocking_signal_set<0> signals{64};
        auto treeHint = bcpp::signal_id{0};
        auto setHint = bcpp::signal_id{0};
        ...
        test_support::require(tree.set(bcpp::signal_id{5}), "tree try-select signal must be publishable");
        test_support::require(signals.set(bcpp::signal_id{7}), "set try-select signal must be publishable");
        treeHint = bcpp::signal_id{0};
        setHint = bcpp::signal_id{0};

        test_support::require(tree.select(treeHint, 0ns) == bcpp::signal_id{5},
                "zero-timeout tree select must consume a ready signal");
        test_support::require(signals.select(setHint, 0ns) == bcpp::signal_id{7},
    ...
    void standalone_tree_timed_select_wakes_for_set()
    {
        ...
        std::jthread waiter
        {
            [&]
            {
                auto hint = bcpp::signal_id{0};
                entered.count_down();
                selected.store(static_cast<bcpp::signal_id::value_type>(tree.select(hint, 5s)));
            }
        };

        entered.wait();
        test_support::require(tree.set(bcpp::signal_id{99}), "standalone tree signal must be publishable");
        waiter.join();
    ...
    void standalone_tree_try_select_is_immediate()
    {
        ...
        test_support::require(tree.set(bcpp::signal_id{11}), "try_select signal must be publishable");
        hint = bcpp::signal_id{0};
        test_support::require(tree.try_select(hint) == bcpp::signal_id{11},
    ...
    void standalone_tree_count_tracks_cardinality()
    {
        ...
        auto first = bcpp::signal_id{1};
        auto second = bcpp::signal_id{2};
    ...
    {
        constexpr auto treeCapacity = bcpp::signal_tree<tree_depth>::capacity;
        blocking_signal_set<tree_depth> signals{treeCapacity * 2};
        auto first = bcpp::signal_id{1};
        auto second = bcpp::signal_id{2};
        auto otherTree = bcpp::signal_id{treeCapacity + 1};
    ...
    void one_transition_wakes_multiple_selectors()
    {
        ...
        for (auto waiter = 0; waiter < waiterCount; ++waiter)
            waiters.emplace_back([&]
            {
                ...
                if (selected.valid())
                {
                    ...
                    selectedMask.fetch_or(1ull << static_cast<bcpp::signal_id::value_type>(selected));
                }
            });
        ...
        for (auto signal = 0; signal < waiterCount; ++signal)
            test_support::require(signals.set(bcpp::signal_id{static_cast<std::uint64_t>(signal)}),
    ...
    void concurrent_shard_transitions_do_not_lose_wakes()
    {
        ...
        for (std::size_t shard = 0; shard < shardCount; ++shard)
            ...
            {
                auto signal = bcpp::signal_id{(shard * treeCapacity) + 7};
        ...
        for (std::size_t consumer = 0; consumer < consumerCount; ++consumer)
            ...
            {
                auto hint = bcpp::signal_id{(consumer % shardCount) * treeCapacity};
                start.arrive_and_wait();
                ...
                for (;;)
                {
                    ...
                    auto selectedValue = static_cast<bcpp::signal_id::value_type>(selected);
    ...
    void repeated_set_select_transitions_do_not_lose_wakes()
    {
        ...
        std::jthread selector
        {
            [&]
            {
                ...
                for (auto repetition = 0; repetition < repetitionCount; ++repetition)
                    if (signals.select(hint, 5s) != bcpp::signal_id{7})
                    {
                        failed.store(true);
                        return;
                    }
        ...
        for (auto repetition = 0; repetition < repetitionCount; ++repetition)
            while (not signals.set(bcpp::signal_id{7}))
                std::this_thread::yield();
    ...
    void repeated_standalone_tree_transitions_do_not_lose_wakes()
    {
        ...
        std::jthread selector
        {
            [&]
            {
                ...
                for (auto repetition = 0; repetition < repetitionCount; ++repetition)
                    if (tree.select(hint, 5s) != bcpp::signal_id{7})
                    {
                        failed.store(true);
                        return;
                    }
        ...
        for (auto repetition = 0; repetition < repetitionCount; ++repetition)
            while (not tree.set(bcpp::signal_id{7}))
                std::this_thread::yield();
